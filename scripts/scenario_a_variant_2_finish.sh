#!/usr/bin/env bash
#
# scripts/scenario_a_variant_2_finish.sh <run-number>
#
# Druga połowa scenariusza A wariant 2. Po zrobieniu i zapisaniu czterech
# screenów w docs/screenshots/scenario-a/variant-2/run-<N>/ skrypt zamyka pull
# request scenariusza, kasuje zdalną gałąź, wraca lokalnie na main i pulluje.
...
echo "============================================================"
echo "SCENARIUSZ A WARIANT 2 RUN ${RUN_N} — FINISH"
echo "============================================================"
...
if [[ -z "${PR_NUM:-}" ]]; then
  ...
  gh pr close "$PR_NUM" \
    --comment "Run ${RUN_N} wariantu 2 ukończony — komplet dowodów zapisany w docs/screenshots/scenario-a/variant-2/run-${RUN_N}/." \
    --delete-branch
fi
...
Następny run:
  bash scripts/scenario_a_variant_2_start.sh $((RUN_N + 1))
