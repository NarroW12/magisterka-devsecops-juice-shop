#!/usr/bin/env bash
#
# scripts/scenario_b_variant_2_finish.sh <run-number>
#
# Druga połowa scenariusza B wariant 2. Po zapisaniu czterech screenów w
# docs/screenshots/scenario-b/variant-2/run-<N>/ skrypt zamyka PR, kasuje
# gałąź, wraca na main.
...
echo "============================================================"
echo "SCENARIUSZ B WARIANT 2 RUN ${RUN_N} — FINISH"
echo "============================================================"
...
if [[ -z "${PR_NUM:-}" ]]; then
  ...
  gh pr close "$PR_NUM" \
    --comment "Run ${RUN_N} wariantu 2 ukończony — komplet dowodów zapisany w docs/screenshots/scenario-b/variant-2/run-${RUN_N}/." \
    --delete-branch
fi
...
Następny run:
  bash scripts/scenario_b_variant_2_start.sh $((RUN_N + 1))
