#!/usr/bin/env bash
#
# scripts/scenario_c_variant_2_finish.sh <run-number>
#
# Druga połowa scenariusza C wariant 2. Po zapisaniu czterech screenów w
# docs/screenshots/scenario-c/variant-2/run-<N>/ skrypt zamyka PR, kasuje
# gałąź, wraca na main.
...
echo "============================================================"
echo "SCENARIUSZ C WARIANT 2 RUN ${RUN_N} — FINISH"
echo "============================================================"

echo "[1/4] Weryfikuję screeny w ${SCREENSHOTS_DIR}"
# Inny ostatni screen niż w wariancie 1 — Checkov detail zamiast Hadolint detail
EXPECTED=(01-pr-blocked.png 02-security-tab.png 03-ci-log.png 04-checkov-detail.png)
MISSING=()
...
if [[ -z "${PR_NUM:-}" ]]; then
  ...
  gh pr close "$PR_NUM" \
    --comment "Run ${RUN_N} wariantu 2 ukończony — komplet dowodów zapisany w docs/screenshots/scenario-c/variant-2/run-${RUN_N}/." \
    --delete-branch
fi
...
============================================================
RUN ${RUN_N} WARIANT 2 ZAKOŃCZONY
============================================================

Następny run:
  bash scripts/scenario_c_variant_2_start.sh $((RUN_N + 1))
