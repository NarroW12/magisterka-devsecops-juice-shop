#!/usr/bin/env bash
#
# scripts/scenario_d_finish.sh <run-number>
...
echo "============================================================"
echo "SCENARIUSZ D RUN ${RUN_N} — FINISH"
echo "============================================================"

echo "[1/4] Weryfikuję screeny w ${SCREENSHOTS_DIR}"
EXPECTED=(01-pr-success.png 02-zap-summary.png 03-zap-alerts.png 04-zap-report-html.png)
MISSING=()
...
if [[ -z "${PR_NUM:-}" ]]; then
  ...
  gh pr close "$PR_NUM" \
    --comment "Run ${RUN_N} ukończony — komplet dowodów zapisany w docs/screenshots/scenario-d/run-${RUN_N}/." \
    --delete-branch
fi
...
Następny run:
  bash scripts/scenario_d_start.sh $((RUN_N + 1))
