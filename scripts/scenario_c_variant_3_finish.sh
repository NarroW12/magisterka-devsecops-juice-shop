#!/usr/bin/env bash
#
# scripts/scenario_c_variant_3_finish.sh <run-number>
...
echo "============================================================"
echo "SCENARIUSZ C WARIANT 3 RUN ${RUN_N} — FINISH"
echo "============================================================"
...
if [[ -z "${PR_NUM:-}" ]]; then
  ...
  gh pr close "$PR_NUM" \
    --comment "Run ${RUN_N} wariantu 3 ukończony — komplet dowodów zapisany w docs/screenshots/scenario-c/variant-3/run-${RUN_N}/." \
    --delete-branch
fi
...
============================================================
RUN ${RUN_N} WARIANT 3 ZAKOŃCZONY
============================================================

Następny run:
  bash scripts/scenario_c_variant_3_start.sh $((RUN_N + 1))
